...
# 🚀 ΔΙΟΡΘΩΣΗ: Βγάλαμε το "print" από εδώ. Είναι μια χαρά Python 3!
_PY2_BUILTINS:    set[str] = {"execfile", "raw_input", "reduce", "reload", "unicode", "basestring", "long", "xrange"}
...
class SpaghettiDetector(ast.NodeVisitor):
    def __init__(self):
        ...
        # 🚀 ΔΙΟΡΘΩΣΗ: Προσθήκη "Μνήμης" για το αν είμαστε μέσα σε Try/Except!
        self.in_try_block      = 0 
    ...
    def visit_Module(self, node):
        ...
        if unused:
            self.file_issues.append(f"Unused Imports: Τα modules/classes {unused} έγιναν import αλλά δεν χρησιμοποιήθηκαν ποτέ.")
        for pkg, reason, lineno in self.sca_issues:
            self.file_issues.append(f"⛔ SCA [γραμμή {lineno}]: Επικίνδυνο/deprecated package '{pkg}' → {reason}")
        for desc, lineno in self.secrets_found:
            self.file_issues.append(f"🔑 Secret Scanning [γραμμή {lineno}]: {desc}")
    ...
    def visit_FunctionDef(self, node):
        ...
        unused_vars = [f"'{var}' (γραμμή {line})" for var, line in self.func_stored_names if var not in self.func_loaded_names and var != '_']

        func_issues = []
        func_start  = node.lineno
        func_end    = getattr(node, 'end_lineno', node.lineno)

        # 🚀 ΔΙΟΡΘΩΣΗ: Αλλάξαμε το God Function από >30 σε >50 γραμμές!
        if self.func_length > 50:
        ...
        if unused_vars:
            func_issues.append(f"Reliability (Unused Vars): Μεταβλητές που δεν διαβάστηκαν ποτέ → {', '.join(unused_vars)}")
        if self.unreachable_lines:
            func_issues.append(f"Reliability (Dead Code): Unreachable κώδικας στις γραμμές: {', '.join(map(str, self.unreachable_lines))}.")
        if self.security_hotspots:
            parts = [f"{vuln} (γραμμή {ln})" for vuln, ln in self.security_hotspots]
            func_issues.append(f"🔐 SAST Security Hotspot: Επικίνδυνος κώδικας → {', '.join(parts)}")
        if self.ai_policy_lines:
            parts = [f"'{name}' (γραμμή {ln}): {msg}" for name, ln, msg in self.ai_policy_lines]
            func_issues.append(f"🤖 AI Policy Violation: {'; '.join(parts)}")
        ...
        for target in node.targets:
            ...
            if _CREDENTIAL_VARNAMES.match(name) and isinstance(node.value, ast.Constant):
                ...
                if len(val) >= 6 and not val.startswith("${") and val.lower() not in {
                    ...
                    self._record_secret(f"Hardcoded credential in variable '{name}' = '{val[:6]}…'", node.lineno)
    ...
    def visit_Call(self, node):
        ...
        if isinstance(node.func, ast.Name) and node.func.id in _UNSAFE_CONVERSIONS:
            ...
            self.security_hotspots.append((f"{func_id}() — {_UNSAFE_CONVERSIONS[func_id]}", lineno))

        for kw in node.keywords:
            if kw.arg == 'shell' and isinstance(kw.value, ast.Constant) and kw.value.value is True:
                self.security_hotspots.append(("subprocess shell=True — OS command injection risk", lineno))
        ...
        if isinstance(node.func, ast.Attribute):
            ...
            if isinstance(owner, ast.Name) and owner.id == "hashlib":
                if attr in _WEAK_HASH_ALGOS:
                    self.security_hotspots.append((f"hashlib.{attr}() — weak/broken hash; use SHA-256+", lineno))
                if attr == "new" and node.args:
                    first = node.args[0]
                    if isinstance(first, ast.Constant) and isinstance(first.value, str):
                        if first.value.lower() in _WEAK_HASH_ALGOS:
                            self.security_hotspots.append((f"hashlib.new('{first.value}') — weak/broken hash; use SHA-256+", lineno))
            if isinstance(owner, ast.Name) and owner.id == "ssl":
                if node.func.attr in _INSECURE_SSL_ATTRS:
                    self.security_hotspots.append((f"ssl.{node.func.attr} — insecure SSL/TLS configuration", lineno))
                ...
                for kw in node.keywords:
                    if kw.arg == "verify_mode" and isinstance(kw.value, ast.Attribute):
                        if kw.value.attr == "CERT_NONE":
                            self.security_hotspots.append(("SSLContext verify_mode=CERT_NONE — certificate verification disabled", lineno))
        ...
        if isinstance(node.func, ast.Attribute) and node.func.attr == "SSLContext":
            if node.args:
                ...
                if isinstance(proto, ast.Attribute) and proto.attr in _INSECURE_SSL_ATTRS:
                    self.security_hotspots.append((f"SSLContext({proto.attr}) — deprecated/insecure protocol", lineno))
            ...
            if node.args and isinstance(node.args[0], ast.Constant):
                ...
                for weak in _WEAK_CIPHERS:
                    if weak in cipher_str:
                        self.security_hotspots.append((f"set_ciphers('{node.args[0].value}') contains weak cipher '{weak}'", lineno))
                        break

        if isinstance(node.func, ast.Name) and node.func.id == "compile":
            if self.current_func:
                self.ai_policy_lines.append(("compile", lineno, _UNSAFE_CONVERSIONS["compile"]))

        # 🚀 ΔΙΟΡΘΩΣΗ: Πλέον ελέγχει αν είμαστε ΕΞΩ από try block (self.in_try_block == 0)
        if isinstance(node.func, ast.Name) and node.func.id in ("int", "float", "complex"):
            if node.args and isinstance(node.args[0], ast.Subscript):
                if self.current_func and self.in_try_block == 0:
                    self.ai_policy_lines.append((
                        node.func.id, lineno,
                        f"Unsafe type conversion {node.func.id}(dict_value) without try/except — will raise ValueError/TypeError on malformed input"
                    ))

        self.generic_visit(node)

    def visit_Attribute(self, node):
        if (isinstance(node.value, ast.Name) and node.value.id == "ssl" and node.attr in _INSECURE_SSL_ATTRS and self.current_func):
            self.security_hotspots.append((f"ssl.{node.attr} referenced — insecure SSL/TLS constant", getattr(node, 'lineno', 0)))
        self.generic_visit(node)
    ...
    # 🚀 ΔΙΟΡΘΩΣΗ: Όταν μπαίνει σε Try block, αυξάνει τον "μετρητή ασφαλείας"
